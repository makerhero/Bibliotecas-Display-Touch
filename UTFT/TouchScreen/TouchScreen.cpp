...
void TouchScreen::read(int *coordinates)
{
  ...
  pinMode( _pinX2, INPUT );     // Analog pin 1
  pinMode( _pinY2, OUTPUT);     // Analog pin 3
  pinMode( _pinY1, OUTPUT );    // Analog pin 0
  digitalWrite( _pinY1 ,LOW );  // Use analog pin 0 as a GND connection
  digitalWrite( _pinY2 ,LOW );
  pinMode( _pinX1 , OUTPUT );    // Analog pin 2
  digitalWrite( _pinX1 ,HIGH ); // Use analog pin 2 as a +5V connection
  _xVal = analogRead( _pinX2 );      // Read the X value
 
 
  // Read the Y coordinate
  pinMode( _pinY1  , INPUT );     // Analog pin 0
  pinMode( _pinX1 , OUTPUT );     // Analog pin 2
  pinMode( _pinX2 , OUTPUT );    // Analog pin 1
  digitalWrite( _pinX2 , LOW );  // Use analog pin 1 as a GND connection
  digitalWrite( _pinX1 , LOW ); 
  pinMode( _pinY2 , OUTPUT );    // Analog pin 3
  digitalWrite( _pinY2, HIGH ); // Use analog pin 3 as a +5V connection
